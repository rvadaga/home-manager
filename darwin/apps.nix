...
let
  ...
  registryEntries = lib.mapAttrsToList (id: app: { inherit app id; }) cfg.registry;
  isEnabled = _: app:
    app.enable && (app.profiles == [ ] || lib.elem cfg.profile app.profiles);
  enabledApps = lib.filterAttrs isEnabled cfg.registry;
  ...
  dockEntries = lib.sort
    (left: right: left.app.dock.position < right.app.dock.position)
  ...
  preferenceEntries = lib.concatMap (entry:
  ...
  appPreferences = lib.listToAttrs (map (entry:
    lib.nameValuePair entry.domain entry.values
  ) preferenceEntries);
  ...
  licenseTargets = map (entry:
    "${entry.app.license.domain}:${entry.app.license.key}"
  ) licenseEntries;
  ...
  manifestSteps = field: lib.concatMap (entry:
    map (text: {
      app = entry.id;
      name = entry.app.name;
      inherit text;
  ...
  manifest = {
    ...
    privacy = manifestSteps "privacy";
    loginItems = manifestSteps "loginItems";
    restore = manifestSteps "restore";
  };

  malformedDockEntries = lib.filter (entry:
    (entry.app.dock.path == null) != (entry.app.dock.position == null)
  ) registryEntries;
  missingDependencies = lib.concatMap (entry:
  ...
  backupBlock = entry:
    let
      ...
      includeFlags = lib.concatMapStringsSep " "
        ...
        backup.includes;
      filterFlags = lib.optionalString (backup.includes != [ ])
        "${includeFlags} --exclude '*'";
...
in {
  ...
  options.personal.apps = {
    ...
    registry = mkOption {
      ...
      description = "central owner of app installation, dock, preferences, setup, license, and backup data";
    };
