...
let
  ...
  instructions = lib.concatMapStrings (platform:
    "\n\n" + builtins.readFile (../dotfiles/claude + "/CLAUDE-${platform}.md")
  ) platforms;
  settingsPieces = map (platform:
    builtins.fromJSON (builtins.readFile (../dotfiles/claude + "/settings-${platform}.json"))
  ) platforms;
in {
  options.llmInstructions = {
    ...
    platforms = lib.mkOption {
      type = lib.types.listOf (lib.types.enum [ "linux" "mac" "nixos" ]);
      default = [ ];
