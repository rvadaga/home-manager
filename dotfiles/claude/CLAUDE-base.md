...
* **before removing a worktree, assume another live session may be working in it too — even the one you are sitting in.** the app can move a running session into a `.claude/worktrees/` directory that already belongs to another running session, with no warning and nothing the session can do about it. so two live sessions can both have the same directory as their cwd, and the app's records say both are right. verified first-hand 2026-07-31: this config's own session was moved `admiring-bohr-9126b4` → `eager-lovelace-054b0a` mid-turn, keeping its branch (so the directory name and the branch inside it no longer matched); its old directory was not deleted but handed to a third session; and the directory it moved into was at that moment still the cwd of a live session, confirmed from both sides. consequences you must assume:
    ...
    * **so removal cannot be made mechanically safe: confirm with rahul per path**, re-deriving the check immediately before acting (claimants change with no observable event). one literal path per command — **never a batch, a loop, or `xargs`** (a PreToolUse hook in `settings-base.json` gates those shapes).
