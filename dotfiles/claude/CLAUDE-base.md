...
* **before you trust a probe that reports "conflict" or "someone else is editing this", re-run it with your own change excluded and see whether the signal survives.** a false alarm blocks a correct change, while a false all-clear costs at most a merge, so the extra run is worth it. most false alarms are the other side being stale: `merge-tree` conflicts often reproduce identically against plain `origin/main`.
