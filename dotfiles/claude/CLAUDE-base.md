...
* **when you write or amend a rule — in CLAUDE.md, a skill, or a doc — check its shape, not only whether it is true.** these defects get past a reader who agrees with every sentence. worked examples for each: `reference-rule-shape-defects.md` in work-home-manager project memory.
    * assume the reader will do the least that technically obeys you, and write so that the least is still correct. a list of options is permission to pick the laziest one, and "check the flagged lines" is obeyed by checking only those while the rest of the file stays broken. say what has to be true rather than naming one example of it.
