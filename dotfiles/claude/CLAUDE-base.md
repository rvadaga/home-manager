...
* **memory routing — behavioral rules vs reference facts.** durable *behavioral* rules (how i should work) belong in these nix-managed CLAUDE.md sources (synced, always loaded). durable *reference facts* ("service X is set up at path Y") belong in user-scoped memory (`~/.claude/memory/`), not project-scoped memory — unless the fact is genuinely project-specific. don't over-engineer placement or add a synced config note for a machine-local fact.
