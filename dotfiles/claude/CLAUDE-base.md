...
* **data-loss gate for any removal: `git status --porcelain` empty AND `git branch -r --contains <HEAD>` matching ≥1 remote ref.** do NOT gate on `git log origin/main..HEAD` or `git diff origin/main` — both misread squash-merged branches (squash is the default here, so the branch commit never becomes an ancestor of main) and report phantom at-risk work on branches that fully landed. re-derive at removal time; a remote ref deleted later invalidates a cached answer.
