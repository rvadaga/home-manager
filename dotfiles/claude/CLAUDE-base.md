...
* **write plainly for every human-facing thing you produce** (docs, PR descriptions, code comments, chat, standups) — default to the register of explaining it out loud to a colleague who just walked up. the fix is NOT "shorter": terse and readable are different axes; a short paragraph dense with coined terms still fails.
    ...
    * name the specific thing, not the category it belongs to. if the word could point at more than one thing here, it is the wrong word: say "waiting for you to approve the merge" rather than "at the gate", and "behind_by is 0" rather than "the measurement". cut whole sentences, not just adjectives.
    ...
    * a question is one sentence plus options a person can choose between without re-reading. if it needs a paragraph of setup, it's the wrong shape — cut the setup, or drop the question and make a recommendation. put it inline where it comes up, never parked at the end of a long report: nobody should have to scroll back to find what you're asking, and search won't find it either.
    * never name a thing that doesn't exist yet. a label for an absent check gives the reader nothing to picture; describe what it would do.
...
* **never put work-internal identifiers into a personal-scoped artifact.** work-internal means anything that only exists inside the job: internal system/tool/skill names, project or ticket ids, page and vault names, repo paths, hostnames, pr numbers, commit shas from a work repo, prod metrics. personal-scoped means anything whose home is outside work — **this repo included**, since it lives on a personal github account even though work configs inherit it.
    * **the destination test is a property, not a list.** ask "could a person outside work read this?", never "is this one of the places i was told about". the rule used to be written only about the personal obsidian vaults, so a base-config skill file — personal repo, inherited by work configs, identical hazard — sailed straight past it. any restatement scoped to one named destination is the bug.
    * **the leak surface is at least three artifacts per change, and they are easy to fix partially:** the file, the pr body, and the **commit message**. the commit message is the expensive one — undoing it needs a rewrite and a force-push, which needs permission — so scrub before committing, not after.
    * write the rule abstractly in personal scope and keep the concrete instances work-side. when a base skill needs real examples, they belong in that skill's work-side `references/` file, which the base skill already reads on work machines.
    * **the check is a grep, so run it** over the added lines, the pr body, and the commit message before pushing — and confirm the pattern matches something it should, or a clean result only proves the grep was broken.
...
* **before removing a worktree, assume another live session may be working in it too — even the one you are sitting in.** the app can move a running session into a `.claude/worktrees/` directory that already belongs to another running session, with no warning and nothing the session can do about it. so two live sessions can both have the same directory as their cwd, and the app's records say both are right. verified first-hand 2026-07-31: this config's own session was moved `admiring-bohr-9126b4` → `eager-lovelace-054b0a` mid-turn, keeping its branch (so the directory name and the branch inside it no longer matched); its old directory was not deleted but handed to a third session; and the directory it moved into was at that moment still the cwd of a live session, confirmed from both sides. consequences you must assume:
    * **a dir that is genuinely your registered cwd may also be a live sibling's cwd.** so "remove only your own worktree" is NOT sufficient — obeying it exactly is how live sessions get destroyed. before removing, ALSO confirm no other non-archived session claims that path (`mcp__ccd_session_mgmt__list_sessions`, exact-path match).
    * **that co-claimant check is necessary but NOT sufficient**, because `cwd` and `branch` from `list_sessions`/`get_session` both go stale on re-binding — a live session can be working somewhere its metadata doesn't name, and a recorded branch can no longer exist. a clean check is "no evidence against", not proof.
    * **the check DECIDES; it is not a prompt. do not ask rahul per path** — asking every time was the old rule and it just moved the same guesswork onto him. **another session claims the path → leave it alone. no claimant but you → remove it.** archived counts as a claimant: an archived session is not a dead one, and treating it as dead is the misclassification that destroyed live sessions' worktrees. re-derive immediately before acting (claimants change with no observable event), one literal path per command — **never a batch, a loop, or `xargs`**. this is on you to follow: a PreToolUse hook enforcing it was tried and reverted, because a hook can only match the command text and so fires on every commit message, pr body, doc and test that merely *quotes* one of these commands — which, on a repo whose docs describe this very rule, is constant.
    * **what makes deciding-without-asking safe is the blast radius, not the check's certainty** — the branch and the pr are the durable objects, so the worst case of a wrong removal is that a live session loses a directory it can recreate with `git worktree add` on the same branch. pair the claimant check with the data-loss gate below and the residual risk is a minor interruption, which is worth not routing every teardown through rahul.
    * **`list_sessions` truncates and you cannot read your way out of it** — it returns at most the limit you pass (measured 2026-08-01: exactly 200 at limit 200, exactly 600 at limit 600), so "returned fewer than the limit" is not available as a completeness proof. it sorts by most recent activity, so truncation drops the STALEST entries first and a live co-claimant would be near the top. a clean scan of a recency-sorted page is therefore adequate evidence about live claimants — proceed on it. verify your matcher with a path you know IS claimed, since a matcher that matches nothing also returns clean.
    * **never `git worktree prune`.** it drops registry entries for worktrees whose dirs are momentarily absent — which under pooled re-binding includes live ones mid-rotation, freeing the path for a later `worktree add` to re-register under a different branch.
* **who may remove a worktree — three cases, and only the middle one stops you.** **(a) self-cleanup:** the path equals your own registered cwd and no other session claims it → remove it. **(b) claimed by any other session, archived or not → leave it alone**, and don't raise it with rahul either; an archived claimant is not a dead claimant, and treating it as one is the misclassification that has destroyed live sessions' worktrees. **(c) reaping** a worktree no session claims at all → remove it. worktrees made by `git worktree add` — what `/nix-rebuild` and `/ship-config` produce — are **born ownerless**: no session's cwd ever equals them, so they always arrive as case (c), which is why (c) has to be autonomous or every ship ends in a question.
    * a session that created a worktree this turn owns its cleanup outright — creating and reaping it inside one task is not "reaping someone else's", whatever the registry says.
* **data-loss gate for any removal — the question is "if this directory vanishes right now, is anything lost?"** answer it with two checks: `git status --porcelain` is empty, AND every commit here survives somewhere else. the second check has two forms and either one is enough on its own:
    * **the branch is still on the remote** — `git branch -r --contains <HEAD>` names ≥1 remote ref. this form expires the moment the branch is deleted, so use it while the branch exists and re-derive it at removal time rather than trusting an earlier answer.
    * **the branch already squash-merged** — `gh pr view <n> --json state,mergedAt,headRefOid` reads `MERGED`, a non-null `mergedAt`, and a `headRefOid` equal to `git rev-parse HEAD`. compare those two shas: `MERGED` on its own says some version of the pr landed, and says nothing about whether the commit in front of you is that version. this form does not expire (a merge does not un-merge) and github keeps `headRefOid` on the pr record after the branch is gone.
    * **the remote-ref form cannot survive a normal ship, which is why the merged form has to exist.** config changes land by squash — the main ruleset here permits no other merge method — so a branch commit never becomes an ancestor of main, and `--contains` asks which remote refs have that commit as an ancestor. the branch is therefore the only ref that can ever match, and `/ship-config` deletes it. both forms measured on this config repo's pr #37 (merged as `e0eafff`, head `c262c30`, branch deleted): `git branch -r --contains c262c30` returns nothing while the same command on `origin/main` returns 3 refs, so the command runs fine and the question was wrong; `gh pr view 37` still reads `MERGED` with that same head sha. `git log origin/main..HEAD` and `git diff origin/main` are reachability walks too and go wrong identically — all three report at-risk work on a branch that fully landed.
    * **what the gate cannot see:** ignored files, which `--porcelain` skips (measured in this repo: 0 lines plain, 3 with `--ignored`) — a `result` symlink or a `.direnv/` cache goes with the worktree, which is intended, but a clean gate does not mean the directory is empty. the merged form also proves only that a sha landed, never that its content was right.
* **losing a worktree is not losing work — the branch and the PR are the durable objects; the worktree is disposable.** if yours disappears or is re-registered, don't reconstruct anything: `git worktree add` a fresh one on the same branch and carry on.
...
* **production correctness, readability, reuse, and long-term maintenance take precedence over avoiding coordinated downstream branch or github-stack refactors.** do not preserve worse production code solely to avoid updating or restacking dependent layers.
...
* **treat the check itself as the thing most likely to be wrong, and remember that proving it ran is not proving it answers your question.** before trusting a probe that reports "conflict" or "someone else is editing this", re-run it with your own change excluded and see whether the signal survives. then ask a second question, precisely because the first one passed: is the thing you are concluding about something this check can actually see? a broken check eventually contradicts something, while a working check answering a slightly different question stays self-consistent forever — `ls-remote` and `gh pr list` correctly returned empty for reserved-but-unpushed pr slots, because a reservation exists in neither tool's world. when the answer is no, shrink the claim to fit what the check saw: "nothing is pushed" was true and useful, "the slots are available" was not.
...
* **when you write or amend a rule — in CLAUDE.md, a skill, or a doc — check its shape, not only whether it is true.** these defects get past a reader who agrees with every sentence. worked examples for each: `reference-rule-shape-defects.md` in work-home-manager project memory.
    ...
    * ask what is unusual about the one case you are generalizing from, or its quirks get baked in as the general rule. if a scheme only holds together once you add an exception clause, treat that clause as a bug report about the idea rather than as something to word carefully.
    * when amending, grep for every place the rule is stated, not just the lines someone flagged. that includes the frontmatter `description`, the one most often missed and the one an agent reads to decide whether a skill applies, and any format strings or examples, which can contradict the prose and are what people copy. broadening a rule can leave its other statements too narrow, so sweep after a change of scope as well as after a correction.
    * a rule written out in full in two places will drift apart. make the second one point at the first, and leave a note saying why it is short, or someone will paste the full text back in.
