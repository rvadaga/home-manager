...
* **"your own worktree" is NOT a reliable idea — the harness re-binds sessions to worktree directories at RUNTIME, silently, carrying the branch across.** verified first-hand 2026-07-31: a session's cwd moved `admiring-bohr-9126b4` → `eager-lovelace-054b0a` mid-turn; the old dir was not deleted but re-registered to a third session's branch; and the dir it moved INTO was simultaneously the cwd of another live session. `.claude/worktrees/*` behaves as a pool. consequences you must assume:
