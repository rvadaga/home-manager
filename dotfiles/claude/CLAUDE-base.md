...
* **before removing a worktree, assume another live session may be working in it too — even the one you are sitting in.** the app can move a running session into a `.claude/worktrees/` directory that already belongs to another running session, with no warning and nothing the session can do about it. so two live sessions can both have the same directory as their cwd, and the app's records say both are right. verified first-hand 2026-07-31: this config's own session was moved `admiring-bohr-9126b4` → `eager-lovelace-054b0a` mid-turn, keeping its branch (so the directory name and the branch inside it no longer matched); its old directory was not deleted but handed to a third session; and the directory it moved into was at that moment still the cwd of a live session, confirmed from both sides. consequences you must assume:
    ...
    * **that co-claimant check is necessary but NOT sufficient**, because `cwd` and `branch` from `list_sessions`/`get_session` both go stale on re-binding — a live session can be working somewhere its metadata doesn't name, and a recorded branch can no longer exist. treat a clean check as "no evidence against", never as proof.
    * **so removal cannot be made mechanically safe: confirm with rahul per path**, re-deriving the check immediately before acting (claimants change with no observable event). one literal path per command — **never a batch, a loop, or `xargs`**. this is on you to follow: a PreToolUse hook enforcing it was tried and reverted, because a hook can only match the command text and so fires on every commit message, pr body, doc and test that merely *quotes* one of these commands — which, on a repo whose docs describe this very rule, is constant.
...
* **who may remove a worktree — three cases, and only one is autonomous.** **(a) self-cleanup:** the path equals your own registered cwd AND no other live session claims it → you may remove it. **(b) claimed by any other session, including an ARCHIVED one → nobody may remove it.** an archived claimant is not a dead claimant; treating it as one is exactly the misclassification that has destroyed live sessions' worktrees. **(c) reaping** a worktree no session claims at all (verify against a `list_sessions` whose returned length is BELOW the limit you passed, or absence means truncation, not death) → allowed only when rahul asks, per path. note worktrees made by `git worktree add` — what `/nix-rebuild` and `/ship-config` produce — are **born ownerless**: no session's cwd ever equals them, so case (a) never applies and they can only ever be reaped.
