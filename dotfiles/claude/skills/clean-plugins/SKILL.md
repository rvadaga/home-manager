...
list all projects where claude code has plugins installed, and clean up plugins not in the `enabledPlugins` list from `~/.claude/settings.json`.
...
4. present a summary:
   ...
   - **wanted plugins:** list from `enabledPlugins` in settings.json
   - **unwanted plugins:** installed plugins whose key is NOT in `enabledPlugins` — grouped by project path
