...
def local_timestamp(milliseconds):
    ...
    return instant.astimezone().isoformat(timespec="milliseconds")


def render(tasks, titles, assignments, projects, projectless):
    lines = ["project  task id  title"]
    for task in tasks:
        ...
        created = local_timestamp(task["created_at_ms"])
        updated = local_timestamp(task["updated_at_ms"])
        lines.append(f"{project}  {task['id']}  {title}")
        lines.append(f"  created {created}  last updated {updated}")
    return "\n".join(lines)
...
def main():
    ...
    print(render(tasks, titles, assignments, projects, projectless))
